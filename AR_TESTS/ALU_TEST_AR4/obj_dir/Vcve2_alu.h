...
#ifndef VERILATED_VCVE2_ALU_H_
#define VERILATED_VCVE2_ALU_H_  // guard

#include "verilated.h"
#include "verilated_cov.h"

class Vcve2_alu__Syms;
class Vcve2_alu___024root;
class VerilatedVcdC;
class Vcve2_alu_cve2_pkg;


// This class is the main interface to the Verilated model
class alignas(VL_CACHE_LINE_BYTES) Vcve2_alu VL_NOT_FINAL : public VerilatedModel {
  private:
  ...
  public:
    ...
    Vcve2_alu_cve2_pkg* const __PVT__cve2_pkg;

    // Root instance pointer to allow access to model internals,
    // including inlined /* verilator public_flat_* */ items.
    Vcve2_alu___024root* const rootp;
    ...
    explicit Vcve2_alu(VerilatedContext* contextp, const char* name = "TOP");
    explicit Vcve2_alu(const char* name = "TOP");
    /// Destroy the model; called (often implicitly) by application code
    virtual ~Vcve2_alu();
  private:
    VL_UNCOPYABLE(Vcve2_alu);  ///< Copying not allowed
